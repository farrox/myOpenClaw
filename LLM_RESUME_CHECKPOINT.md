...
- [x] **Household agent `partner`** — Workspace **`~/clawd-partner`**; identity **Partner assistant** / ✨; exec allowlist mirrors **`main`**; **`auth-profiles.json`** copied from **`main`** (replace if the second user should have separate billing).
...
- [ ] **Partner workspace + routing** — Customize **`~/clawd-partner/TOOLS.md`**; add **`openclaw agents bind`** if channels should route to **`partner`** vs **`main`**; **restart gateway** after config changes.
...
2. **Partner assistant** — Finish **`~/clawd-partner/TOOLS.md`**; **`openclaw agents bindings`** / **`bind`** if needed; separate **`auth-profiles.json`** for **`partner`** if you want a different API key (see **[UBUNTU_AGENT_HOUSEHOLD_GUIDE.md](UBUNTU_AGENT_HOUSEHOLD_GUIDE.md)**).
...
*Last checkpoint update: 2026-04-11 — **`main`** display **Assistant** / 🦞; second agent **`partner`** (`~/clawd-partner`); **`tina`** removed.*
